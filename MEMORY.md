...
**Decision (supersedes the 2026-07-20 "credit-based Benefits/loyalty... walled-off rail" entry above)**: Benefits/Loyalty is reframed entirely to a discount-code model — no benefit-specific credit currency at all. Tier eligibility is activity/tenure-based (mirrors existing points-earning); the benefit is a generic, non-tracked discount code; the user pays the vendor directly on the vendor's own payment page; JumpOnBoard never touches payment. Applies uniformly across all categories. Legal status downgrades from hard blocker to confirmation-only.
...
**Decision**: Rebranded **JumpOnBoard → Binding** as the public brand; "JumpOnBoard" is retired as an internal code name. Domain strategy: `binding.hk` for MVP (HK$250/yr registry fee), `.com` deferred. No architecture change — technical identifiers (`@jumponboard/ui` package, Vercel project `jumponboard-staging`, staging URL, docker stack name) deliberately kept as-is to avoid breaking infrastructure.
...
## 2026-08-12 (strategy/design expansion pass — 10 founder items, 11 grilling rounds)

**Decision**: Client-held key custody (envelope encryption for the original resume upload, DESIGN.md §2g) is reframed from "extra encryption" to an **AI-processing consent/audit gate** — a guarantee that the redaction/embedding LLM cannot touch raw PII without an explicit, per-operation authorized decrypt event, applying inside the enterprise customer-VPC boundary too (not just at the B2C edge).
**Context**: The founder asked for client/passkey-held private keys, decrypt-only-on-auth, framed initially as a privacy hardening measure. §2f had already rejected "browser E2EE with **server-held** keys" as theater — genuinely client-held keys are a different, un-rejected design, but the real problem was reconciling live-auth-only decrypt with AI processing that needs plaintext, and with item 6's autonomous background agent (which can't be present for a live ceremony). Resolved via envelope encryption (N wrapped copies of one data key; server never holds an unwrapped key) plus the finding that the background/periodic-nudge loop (§2c) only ever needs the derived/redacted working copy, never the encrypted original — so an agent can queue a suggestion without ever touching ciphertext.
**Outcome/Lesson**: The enterprise application of this mechanism is the more valuable finding than the B2C one — inside a customer's own VPC, the VPC boundary alone doesn't stop in-VPC AI models from processing raw data without authorization; §2g closes that gap and becomes a concrete, exportable, sellable governance artifact (BUSINESS §6/§7.3) rather than a B2C-only hardening. Deliberately **no break-glass exception** — an absolute "no LLM touches raw PII without authorization, ever" is the actual claim being sold, and a break-glass path would undermine it. Total key-loss (device + recovery codes both gone) is accepted as permanent, disclosed data loss rather than solved via escrow, for the same reason.
**Links**: DESIGN.md §2f/§2g, BUSINESS.md §6/§7.3, LEGAL_REVIEW.md.

**Decision**: Open-ended AI-graded Q&A supersedes DESIGN.md §13d's auto-scored MCQ as the **default** going-forward skill-assessment design (DESIGN.md §14b) — not merely a future alternative kept in roadmap alongside MCQ.
**Context**: An earlier draft of this plan, written before founder review, got this backwards — it recorded "keep MCQ for MVP, open-ended Q&A as next-iteration roadmap" when the founder had actually picked open-ended Q&A as the default and work-sample analysis as the roadmap-parked "nice to have." Caught during the advisor review pass before the plan was presented, not after.
**Outcome/Lesson**: When a multiple-choice question's options don't cleanly map to "keep as-is" vs "replace," re-read the literal answer text before summarizing it into a decisions table — "option 1 as default, option 2 nice to have" is not the same as "option 3 (status quo) for now." Also: replacing §13d's grading mechanism (deterministic MCQ → per-attempt AI) reopens the "no per-attempt human-in-the-loop" claim §13d made — resolved via a founder-reviewed rubric bank (mirroring the reviewed-question-bank pattern) plus restated anti-farming mitigations, not by silently inheriting §13d's old justification.
**Links**: DESIGN.md §13d/§14b.

**Decision**: AI Company Research (a candidate-facing public-source company summary, DESIGN.md §14k) is a standalone first-party feature, explicitly NOT routed through the AI-Credit-Marketplace/career-assistant allowance (BUSINESS §3 pillar 5, DESIGN §7).
**Context**: An earlier draft folded it in as "just another example task" inside that allowance's classifier-gated scope, reasoning it avoided new monetization infrastructure. Advisor review flagged this as the actual blocker in that draft: the allowance's narrow, closed-list scope (resume rewriting, cover letters, interview prep, career-path guidance — text generation only) is exactly what BUSINESS §11 relies on to keep its legal status confirmation-only rather than a hard blocker; adding live public-web retrieval to what the redeemable key can invoke would have widened that scope without anyone deciding to.
**Outcome/Lesson**: A feature that "reuses existing infrastructure for economy" can silently widen the legal scope that infrastructure's status depends on — check what a scope-narrowing legal argument (LEGAL_REVIEW.md, BUSINESS §11) actually enumerates as in-scope before folding a new capability into it, even when the new capability seems small and the existing mechanism seems like a natural fit.
**Links**: DESIGN.md §14k, BUSINESS.md §3/§11, LEGAL_REVIEW.md.

**Decision**: The Enterprise Performance & Relations module's peer-review replacement (DESIGN.md §14h) uses AI synthesis from **employee-submitted** project data only, not passive monitoring of git/Slack/Jira/OKR tools — the "lighter-touch" option over full work-tool integration.
**Context**: The founder's item 1 rejected "scorecard for peer review" as too traditional; the only plausible target was this not-yet-designed enterprise module (BUSINESS §3a cluster 4). A full passive-monitoring version was considered and explicitly not chosen, specifically to avoid employee-surveillance/monitoring-notice exposure under HK/SG workplace-privacy expectations.
**Outcome/Lesson**: "Reduce human input" as a general design directive needs a per-feature check against who the input is coming from and what replaces it — for a consumer-facing feature, less manual typing is a UX win; for an employer-facing performance-review feature, "less human input" read as "the AI reads your Slack messages" would trade one problem (traditional-feeling scorecards) for a worse one (surveillance exposure).
**Links**: DESIGN.md §14h, BUSINESS.md §3a, LEGAL_REVIEW.md Part 7.

**Decision**: Most-restrictive-wins is the conflict-resolution default when a user's rule-based agent auto-reply rules (DESIGN.md §14e) conflict with each other, and an account-level kill switch exists separately from per-rule revoke.
**Context**: Item 6 asked for "granular authorization" letting an agent reply on behalf of the user. Founder confirmed human-in-the-loop stays on for profile edits but chose rule-based pre-authorized actions (not draft-only, not full agent judgment) for replies — narrower than open-ended autonomy but still capable of two rules firing on the same event with opposite outcomes.
**Outcome/Lesson**: Any "narrow, deterministic, user-authored rules" design for autonomous action still needs an explicit conflict-resolution default and a panic button, even when each individual rule is simple — the failure mode worth defaulting against is an accidental auto-accept/auto-disclosure, not an accidental auto-decline.
**Links**: DESIGN.md §14e.
