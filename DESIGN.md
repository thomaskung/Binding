...
**Version 2.0** · Last updated 2026-07-28 · Revision history at the end of this document.
...
- **Compliance-ops execution items (decided 2026-07-28)** — before private beta: signed subprocessor DPAs (Modal, Supabase, Cloudflare), subprocessor register + privacy-notice page (`/privacy`, draft-for-counsel), manual DSAR/withdrawal runbook (`docs/DSAR_RUNBOOK.md` — PDPO's 40-day access-request clock binds from the first real user). Before public launch: account-deletion self-service (§11).
...
- **Recruiter monetization pricing ceiling** (BUSINESS.md §7): the ~20%-of-commission design guideline has no enforcement mechanism today — JumpOnBoard doesn't track placements or commission amounts. It calibrates suggested per-role budget defaults (§4a) only; treat as a heuristic, not a constraint the entitlement logic checks against.
...
- Account-deletion self-service (before public launch — §2f/§5): needs the anonymize-not-delete ledger design (append-only `points_ledger` rows stay as the counterparty's accounting record; PII columns null out — see `docs/DSAR_RUNBOOK.md` for the manual procedure it automates).
