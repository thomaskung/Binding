  ...
  const [{ data: profile }, { data: consent }, { data: experience }, balance, { data: ledger }, { data: auth }] =
    await Promise.all([
      supabase
        .from("profiles")
        .select(
          "display_name, draft_text, published_text, visibility, dealbreaker_matrix, headline, phone, location, skills, desired_roles, industries, references_available, share_salary, credentials, credentials_summary, field_visibility",
        )
