Executable Strategy Plan: "Binding"

**Version 2.1** · Last updated 2026-08-04 · Revision history at the end of this document. Companions: DESIGN.md (technical), VISION.md (goals), MEMORY.md (decision log).
...
3. The Solution: The Binding Ecosystem

Binding creates a "dark pool" of passive, high-intent talent using advanced AI and a closed-loop points economy — no cash vouchers, no stored monetary value, by design (see §7 and §11 for why this matters legally).
...
Ecosystem Lock-in (Points): Users actively lose accumulated platform value (points) if they abandon their Binding profile, creating strong retention. Combined with in-platform messaging/scheduling and the AI-Credit Marketplace, retention is built on product stickiness ("hurdles") rather than contractual restriction ("blockers") — see §3.
...
7. Competitor Pricing & Binding Revenue Model

To capture market share efficiently, Binding utilizes a Tri-Tier Freemium model, benchmarked against incumbent platforms.
...
Binding Pricing Structure (all prices in USD):

**Build-state note (pricing is designed, not yet billable)**: no payment processor is integrated today — every priced tier below (Pro Seeker $9.99, Solo $99, Pro SaaS $599, point top-ups) is a *designed* model, not a live checkout. What is built: the closed-loop points ledger, standard + override reveals at **flat** placeholder costs (`REVEAL_COST=10` / `OVERRIDE_COST=25`), and daily caps. The §7-item-2 recruiter mechanics (per-role budgets, match-quality-tiered dynamic pricing, recruiter-initiated override reversal, paid AI JD-assist) are roadmap (DESIGN.md §4a) — the AI JD-assist is currently free. Billing integration is deferred (post-demo); the current milestone is an investor/grant demo, not revenue.
...
Frontend: Next.js (React) web application. In-platform messaging is built (a core retention mechanic, §3); in-platform interview scheduling is roadmap (table-ready, UI not yet built).
...
Layered privacy architecture — **"Privacy at the edge, Security at the core"** (target architecture, DESIGN.md §2f): Layer 0 client-side controls (built), Layer 1 per-jurisdiction "edge" holding raw PII + redaction compute (HK/SG regional stores for B2C; a customer-VPC variant for enterprise B2B where raw employee data never leaves the client's environment and only pseudonymized vectors flow to the core under DPA — a named trade-off, not hidden), Layer 2 Singapore-based matching core over pseudonymized data only, Layer 3 internal-access controls (masked-by-default, break-glass, audited). MVP runs single-region with the same boundaries enforced logically by RLS; physical split is gated on named triggers (first enterprise deal, ESS grant application, residency-mandating market entry). This is also the enterprise B2B data-residency pitch (§7.3) — adopted after research rejected the ZK-FL alternative (DESIGN.md §2f).
...
Use traction generated by headhunters placing Binding candidates to sell the Enterprise product directly to corporate HR teams.

9a. Government-Grant Funding Path (HK, laddered — added 2026-07-28)

Non-dilutive HK government funding is a planned funding path (founder-confirmed 2026-07-28), structured as a **ladder, not an ESS-first plan**:

1. **Cyberport CCMF / HKSTP Ideation** (~HK$100K, no matching-fund requirement) — the realistic pre-seed rung.
2. **Incubation programmes** (Cyberport Incubation / HKSTP Incubation) — post-traction.
3. **ITF Enterprise Support Scheme (ESS)** — at Series-A scale only: ESS is **1:1 matching** (an HK$10M grant requires HK$10M of the company's own capital), so it is not a pre-seed instrument, whatever a pitch-deck-friendly reading suggests.

Requirements recorded as **directional and unverified against primary ITC/Cyberport/HKSTP sources** (same discipline as §11's secondary-source items): ESS R&D must be conducted primarily in Hong Kong; outside-HK spend needs prior CIT approval; the Research Talent Hub can subsidize local R&D hires; and — the solo-founder-specific point — R&D spend is mostly founder salary, so **founder HK residency/payroll is an ESS precondition to verify**. Engage a grant consultant before any application.

**Grant scope = the HK edge layer** (DESIGN.md §2f): the HK opco owns, develops, and hosts the edge layer (redaction pipeline, HK regional store, HK GPU serving) — that is the ESS-claimable, HK-domiciled R&D. SG-core development is not claimed. MVP stays on Modal (now region-pinned APAC — DESIGN.md §12) for cost reasons; edge-layer physicalization including HK GPU serving is an ESS-application precondition, not an MVP task.
...
**Training credits** (DESIGN.md §7a) — a separate, narrow, one-way (points→credit only) redemption instrument for Binding's own training content, with enterprise-purchased seats segregated from personal credits. Legal status: **confirmation-only**, with the enterprise cash-purchase path flagged specifically for counsel (cash-in → held balance → later redemption is the classic prepaid/stored-value trigger pattern; narrow same-service redemption is the defense).
...
Re-identification Risk: Acknowledged as a residual, non-zero risk (see §3, Pillar 1) rather than designed around an absolute anonymity claim. Mitigated via redaction, quasi-identifier generalization, and rate-limited reveals (both reveal paths daily-capped as of 2026-07-28); not eliminated. **Embedding-inversion risk** (stored skill vectors can be partially inverted back to text — PROBLEM.md Problem 1) is documented in DESIGN.md §5: redaction-before-embedding bounds it to already-redacted text, vectors never leave the database, and DP-noise hardening is a deferred-with-trigger item.

Automated Profiling & Accuracy (PDPA/PCPD — added 2026-07-28): consent is now granular (processing + automated-profiling required, continuous-maintenance optional/withdrawable — DESIGN.md §2a, counsel confirmation at LEGAL_REVIEW.md Q14), and the PDPA Accuracy Obligation is met structurally: every AI-drafted profile change is suggest-and-approve, so committed data was explicitly user-approved (Q15). Compliance-ops execution items (subprocessor DPAs, privacy notice, DSAR runbook, deletion self-service) are scheduled in DESIGN.md §5.
...
Domain & Brand: We keep the "Binding" name. We have acquired **`getbinding.com`** and launch under it for MVP; a `.hk` may be added later for the HK market. (The HK grant/nexus story rides on the HK opco, HK-domiciled R&D, and HK market focus — not the domain TLD.)
...
| 1.8 | 2026-07-28 | PROBLEM.md remediation pass (founder-grilled, twice advisor-reviewed — companion to DESIGN.md 2.0). New §9a: laddered HK grant path (CCMF/HKSTP Ideation → incubation → ESS at Series-A; ESS 1:1-matching reality and founder-residency precondition named; HK edge layer = the grant-scoped R&D). §8: four-layer "Privacy at the edge, Security at the core" target architecture summary + enterprise VPC-edge pitch (vectors-leave/raw-stays trade-off named). §11: embedding-inversion pointer, granular-consent + Accuracy-Obligation posture, both reveal paths now daily-capped. ZK-FL (PROBLEM.md's Problem-5 fix) rejected after adversarial deep-research — see DESIGN.md §2f. |
| 1.9 | 2026-07-30 | Staging E2E test infrastructure shipped. Staging gated behind HTTP Basic Auth + `x-staging-auth` shared-secret header (defense-in-depth, env-gated — invisible in local dev). Account deletion implemented as full privacy lifecycle (cascade delete, Supabase Storage cleanup, points ledger sanitization, job posting soft-close). 17 functional + 7 UAT rubric-based tests run nightly against staging (3am UTC) with OpenCode dual-agent consensus scoring against BUSINESS.md promises. Weekly cleanup deletes stale test users. Repo secrets policy enforced: all credentials in `.env.local` or GH Actions encrypted secrets, never in code. |
| 2.0 | 2026-08-03 | Brand rename: **JumpOnBoard → Binding** (public brand; "JumpOnBoard" was the internal code name and is retired). Domain + Cyberport-CCMF prep (both partly corrected in 2.1 below). |
| 2.1 | 2026-08-04 | **Demo-readiness reconciliation pass** (investor/CCMF-demo milestone): corrected doc-vs-code drift so every "built" claim matches the codebase. Domain corrected `binding.hk` → **`getbinding.com`** (the acquired domain) in §12 + `/privacy` + DSAR runbook (contact `privacy@getbinding.com`). Interview scheduling downgraded from "core MVP" to roadmap (§3/§8 — table exists, no UI). §7 gains a build-state note: no payment processor exists, every priced tier is a designed-not-billable model, reveal costs are flat placeholders, and the §4a recruiter mechanics (per-role budgets, dynamic pricing, override reversal, paid JD-assist) are roadmap. "Seekers Earn" build-state clarified — only training-completion is a wired seeker-initiated earn; skill-assessment flow is roadmap. Revision history reordered (1.7 was out of sequence). Companion to DESIGN.md 2.4. Note: `docs/CCMF-Application-Binding.md` referenced in 2.0/MEMORY is not present in the repo — flagged for the founder. |
